(() => {
  ...
  var import_meta, BUILDING_BLOCK_atomRead, BUILDING_BLOCK_atomWrite, BUILDING_BLOCK_atomOnInit, BUILDING_BLOCK_atomOnMount, BUILDING_BLOCK_ensureAtomState, BUILDING_BLOCK_flushCallbacks, BUILDING_BLOCK_recomputeInvalidatedAtoms, storeMutationSet, BUILDING_BLOCK_readAtomState, BUILDING_BLOCK_invalidateDependents, BUILDING_BLOCK_writeAtomState, BUILDING_BLOCK_mountDependencies, BUILDING_BLOCK_mountAtom, BUILDING_BLOCK_unmountAtom, BUILDING_BLOCK_setAtomStateValueOrPromise, BUILDING_BLOCK_storeGet, BUILDING_BLOCK_storeSet, BUILDING_BLOCK_storeSub, BUILDING_BLOCK_registerAbortHandler, BUILDING_BLOCK_abortPromise, buildingBlockMap, getInternalBuildingBlocks;
  var init_internals = __esm({
    "node_modules/jotai/esm/vanilla/internals.mjs"() {
      import_meta = {};
      BUILDING_BLOCK_atomRead = (_store, atom2, ...params) => atom2.read(...params);
      BUILDING_BLOCK_atomWrite = (_store, atom2, ...params) => atom2.write(...params);
      BUILDING_BLOCK_atomOnInit = (store, atom2) => {
        var _a;
        return (_a = atom2.INTERNAL_onInit) == null ? void 0 : _a.call(atom2, store);
      };
      BUILDING_BLOCK_atomOnMount = (_store, atom2, setAtom) => {
        var _a;
        return (_a = atom2.onMount) == null ? void 0 : _a.call(atom2, setAtom);
      };
      BUILDING_BLOCK_ensureAtomState = (store, atom2) => {
        var _a;
      ...
      BUILDING_BLOCK_flushCallbacks = (store) => {
        const buildingBlocks = getInternalBuildingBlocks(store);
      ...
      BUILDING_BLOCK_recomputeInvalidatedAtoms = (store) => {
        const buildingBlocks = getInternalBuildingBlocks(store);
        const mountedMap = buildingBlocks[1];
      ...
      storeMutationSet = /* @__PURE__ */ new WeakSet();
      BUILDING_BLOCK_readAtomState = (store, atom2) => {
        var _a, _b;
        const buildingBlocks = getInternalBuildingBlocks(store);
        const mountedMap = buildingBlocks[1];
        const invalidatedAtoms = buildingBlocks[2];
      ...
      BUILDING_BLOCK_invalidateDependents = (store, atom2) => {
        const buildingBlocks = getInternalBuildingBlocks(store);
        const mountedMap = buildingBlocks[1];
        const invalidatedAtoms = buildingBlocks[2];
        const ensureAtomState = buildingBlocks[11];
      ...
      BUILDING_BLOCK_writeAtomState = (store, atom2, ...args) => {
        const buildingBlocks = getInternalBuildingBlocks(store);
        const changedAtoms = buildingBlocks[3];
        const storeHooks = buildingBlocks[6];
      ...
      BUILDING_BLOCK_mountDependencies = (store, atom2) => {
        var _a;
        const buildingBlocks = getInternalBuildingBlocks(store);
        const mountedMap = buildingBlocks[1];
        const changedAtoms = buildingBlocks[3];
      ...
      BUILDING_BLOCK_mountAtom = (store, atom2) => {
        var _a;
        const buildingBlocks = getInternalBuildingBlocks(store);
        const mountedMap = buildingBlocks[1];
        const mountCallbacks = buildingBlocks[4];
      ...
      BUILDING_BLOCK_unmountAtom = (store, atom2) => {
        var _a, _b;
        const buildingBlocks = getInternalBuildingBlocks(store);
        const mountedMap = buildingBlocks[1];
        const unmountCallbacks = buildingBlocks[5];
      ...
      BUILDING_BLOCK_setAtomStateValueOrPromise = (store, atom2, valueOrPromise) => {
        const buildingBlocks = getInternalBuildingBlocks(store);
        const ensureAtomState = buildingBlocks[11];
      ...
      BUILDING_BLOCK_storeSub = (store, atom2, listener) => {
        const buildingBlocks = getInternalBuildingBlocks(store);
        const flushCallbacks = buildingBlocks[12];
        const mountAtom = buildingBlocks[18];
        const unmountAtom = buildingBlocks[19];
      ...
      BUILDING_BLOCK_registerAbortHandler = (store, promise, abortHandler) => {
        const buildingBlocks = getInternalBuildingBlocks(store);
      ...
      BUILDING_BLOCK_abortPromise = (store, promise) => {
        const buildingBlocks = getInternalBuildingBlocks(store);
        const abortHandlersMap = buildingBlocks[25];
  ...
  var import_meta2, keyCount, overriddenCreateStore;
  ...
  var sql_wasm_browser_default;
  var init_sql_wasm_browser = __esm({
    "node_modules/sql.js/dist/sql-wasm-browser.wasm"() {
      sql_wasm_browser_default = "./sql-wasm-browser-SYJ4KWY2.wasm";
    }
  });
...
`;
      ...
      loadChatState = (db) => {
        ...
        const activeMessages = (activeRows || []).map((r) => ({
          role: r[0],
          content: r[1],
          createdAt: r[2]
        }));
        ...
        const archivedChats = (chatRows || []).map((r) => {
      ...
      savePersistStateToLocalDb = (SQL, db, persistState) => {
  ...
  var require_app = __commonJS({
    "public/app.ts"() {
      ...
      var chatForm = getEl("chatForm");
      var messageInput = getEl("messageInput");
      var chatLog = getEl("chatLog");
      var sendButton = getEl("sendButton");
      var newChatButton = getEl("newChatButton");
      var recentList = getEl("recentList");
      var sideMenu = getEl("sideMenu");
      ...
      var getCurrentChatListItem = (chatState) => {
      ...
      var renderRecent = () => {
        ...
        combined.forEach((chat) => {
          ...
          meta.textContent = `${chat.messagesCount} msgs \xB7 ${formatDate(chat.createdAt)}`;
      ...
      var pushUiSystemMessage = (content) => {
        setUi((prev) => ({
          ...prev,
          uiMessages: [...prev.uiMessages, { role: "system", content, createdAt: (/* @__PURE__ */ new Date()).toISOString() }]
        }));
      };
      var boot = async () => {
      ...
      recentList.addEventListener("click", async (event) => {
        ...
        if (deleteBtn instanceof HTMLElement) {
          ...
          const item2 = deleteBtn.closest(".archive-item");
          if (!(item2 instanceof HTMLElement)) return;
          const chatId2 = item2.dataset.chatId;
          if (!chatId2) return;
          if (!confirm("Remove this chat from your history on this device?")) return;
          const persistState2 = store.get(persistStateAtom);
          store.set(persistStateAtom, {
            ...persistState2,
            chatState: {
              ...persistState2.chatState,
              archivedChats: persistState2.chatState.archivedChats.filter((c) => c.id !== chatId2)
            }
          });
          return;
        }
