// Source: https://vsellm.ru/provider/VSELLM (LLM tab, Jul 2026)
export type VsellmCatalogEntry = { id: string; name: string; provider: string };

export const VSELLM_TEXT_MODELS: VsellmCatalogEntry[] = [
...
export const getVsellmProviderLabel = (provider: string): string =>
  PROVIDER_LABELS[provider] ?? provider;

export const getVsellmCatalogEntry = (modelId: string): VsellmCatalogEntry | undefined =>
  VSELLM_TEXT_MODELS.find((entry) => entry.id === modelId);

export const formatVsellmModelOptionLabel = (entry: VsellmCatalogEntry): string =>
  entry.name.includes(":") ? entry.name : `${getVsellmProviderLabel(entry.provider)}: ${entry.name}`;

export const compareVsellmCatalogEntries = (
  a: VsellmCatalogEntry,
  b: VsellmCatalogEntry
): number => {
...
export const getSortedTextModels = (): VsellmCatalogEntry[] =>
  [...VSELLM_TEXT_MODELS].sort(compareVsellmCatalogEntries);
